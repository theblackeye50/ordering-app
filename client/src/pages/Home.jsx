...
const Home = () => {
  ...
  if (!subcategory) {
    ...
    // fallback: just go to the category page
   ...
   <section className='bg-white'>
      ...
      {/* {
        categoryData?.map((c,index)=>{
         ...
         {/* Display products for each category by chatgpt suggestion */}
      {categoryData?.map((c) => {
  const subcategory = subCategoryData.find(sub => sub.category?.some(cat => cat._id === c._id));

  return (
    <CategoryWiseProductDisplay
      key={c._id + "CategorywiseProduct"}
      id={c._id}
      subCategoryId={subcategory?._id}  // ✅ pass subcategory ID too
      name={c.name}
    />
  );
})}
